...
mean.betahats <- stderr.betahats <- c()
post.betahats <- list(samples[[1]][,'beta.hat[1]'], samples[[1]][,'beta.hat[2]'], samples[[1]][,'beta.hat[3]'], samples[[1]][,'beta.hat[4]'], 
                      samples[[1]][,'beta.hat[5]'], samples[[1]][,'beta.hat[6]'], samples[[1]][,'beta.hat[7]'], samples[[1]][,'beta.hat[8]'])

for(i in 1:length(post.betahats)){
  mean.betahats   <- c(mean.betahats,   round(mean     (post.betahats[[i]]),3) )
  stderr.betahats <- c(stderr.betahats, round(std.error(post.betahats[[i]]),3) )
}
...
table(YY$lambda)
round(table(YY$lambda.hat_per_id)/dim(data.valid)[1], 3)
