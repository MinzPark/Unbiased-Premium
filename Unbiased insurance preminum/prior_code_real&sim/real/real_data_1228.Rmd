...
data.train$C3  <- ifelse(data.train$col.Cov.Idx==3, 1, 0)
```
...
# Y[,tau+1]에 대한 값이 data.valid에 있으므로 data.train으로 추출한 Y는 향후 data.train에서 합쳐야함.
# Y[,tau+1]에 ID를 넣어주고 1~tau까지 Y값을 ys에 넣어서 Y에 넣어줌. 
for(i in 1:length(id_uniq)){
  Y[i,tau+1] = id_uniq[i]
  ys = data.train[data.train$PolicyNum == id_uniq[i], ]$n # 각 ID에 대한 1~tau의 y값들을 추출함. 
...
> total matrx의 col = ID, lambda_vecotr, R, y[1:5], y[6]에 해당하는 n <br/>
> 그러므로 Y[N][6]인 Y의 데이터 프레임, lambda vector를 따로 추출해서 향후 사용  <br/>
...
total <- merge(Y.df, data.valid[,c('PolicyNum','n')], by.x = "ID", by.y = 'PolicyNum') # tau+1의 Y값이 있는 data.valid를 이용해 ID로 병합함. 
...
a = a.hat; N_sim = dim(total)[1]; tau = 5; n_model = 3; lam_vec <- total[,'lambda']; k = k.hat

table(total[,'lambda']); table(lam_pol); # 1~tau까지 병합된 Y데이터에대해 lambda_vector 각 값들과 1~tau까지 있는 409명에 대한 lambda_vector 값
