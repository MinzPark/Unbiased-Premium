...
##########chk which difference btw Ohrosy and us conclusion?
# 1. comparison of ratio of each parameter
# 각 x의 variable이 차지하는 percentage가오로지 교수님과 약간씩 차이남.
# 별로 차이가 크진 않으나, 약간씩 차이를 보임. -> 유의미한 이유라고 생각되지 않음.
# 2. beta.hat의 추정 범위가 너무 큰거 아닐까? -> sd를 1로 줄여도 같은 결과 도출 x
...
source("C:/Users/none/Desktop/대학원/논문/Unbiased insurance preminum/Pois_Gamma_RE.R") 
...
png(filename=paste0(path,'/Pois_Gamm_real_data_alpha0.png'),width=600,height=600)
...
# real data의 경우 MSE만 산출함. (왜냐면 real data에선 R이 없음.)
MSE <- rep(0, n_model)
for(i in (1:n_model)){
