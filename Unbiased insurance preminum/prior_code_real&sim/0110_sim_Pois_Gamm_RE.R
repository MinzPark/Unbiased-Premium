...
source(paste0(path,"/Pois_Gamma_RE.R"))
...
for(i in 1:(dim(total_lam)[1])){
  # save param
  l <- list( N_sim = NA, tau = tau, lam_ = NA, a = NA, s_hmse = NA, c_hmse = NA, gc_hmse = NA, c_dix = NA)
  
  
  # #lambs = c(0.1, 0.2, 1.2); 
  lambs = total_lam[i,];
  a = 0.8; N_sim = 10000; n_model = 3
  ...
  new_folder_name <- paste0(path,"\\m3_alpha0_plot\\", format(Sys.Date(),"%m%d"),"_Pois_Gamm")
  ...
  # to save each case 
  l['N_sim'] <- N_sim; l['tau'] <- tau; l['lam_'] <- paste(lambs, collapse = ","); l['a'] <- a
...
write.csv(df_tau, file = paste0(path,filename))
